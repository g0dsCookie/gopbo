...
if (getNumber(configFile >> "CfgVehicles" >> typeOf _object >> "exileIsLockable") isEqualTo 1) then
{
	...
	{
		[_sessionID,"systemChatRequest",["Access denied!"]] call ExileServer_system_network_send_to;
	}
	else
	{
		...
		else
		{
			...
			else
			{
				...
				switch (_numberOfFails) do
				{
					case 1:
					{
						[_sessionID,"systemChatRequest",["Wrong PIN code! Two tries remaining."]] call ExileServer_system_network_send_to;
					};
					case 2:
					{
						[_sessionID,"systemChatRequest",["Wrong PIN code! One try remaining."]] call ExileServer_system_network_send_to;
					};
					default
					{
						[_sessionID,"systemChatRequest",["Wrong PIN code! Access denied for five minutes."]] call ExileServer_system_network_send_to;
